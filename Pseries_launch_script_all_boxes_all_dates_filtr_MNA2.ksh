...
rm -f fdr1_filtr2_${dtime[$SLURM_ARRAY_TASK_ID - 1]}.dat
for k in $(seq 31 66); do 
        km=$(echo $(( $k - 1)))
	#echo "python extractions-gradients-all-variables-boxes.py 'LS' $km $date" > extract-boxLS${km}-${date}.ksh
        ...
        echo "python extractions-gradients-all-variables-boxes-2019-11-13.py 'MNA' $km ${dtime[$SLURM_ARRAY_TASK_ID - 1]}" >> fdr1_filtr2_${dtime[$SLURM_ARRAY_TASK_ID - 1]}.dat
done
...
which python
srun -n $SLURM_NTASKS pserie_single < fdr1_filtr2_${dtime[$SLURM_ARRAY_TASK_ID - 1]}.dat
